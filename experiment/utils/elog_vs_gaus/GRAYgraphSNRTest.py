...
def plot_values(iteration):
    ...
    plt.tight_layout()
    #fig = plt.gcf()
    fig.savefig(f"elog_vs_gaus/gray_plot_iter:{iteration}")
    ...
    axs[1,0].set_title(f'ELoG-Filter {iteration} Iterations')
    axs[1,0].axis('off')

    # axs[1,1].imshow(gaus_image)
    axs[1,1].imshow(gaus_image, cmap='gray')
    axs[1,1].set_title(f'Gaussian-Filter {iteration} Iterations')
    axs[1,1].axis('off')
    ...
    plt.tight_layout()
    #fig = plt.gcf()
    fig.savefig(f"elog_vs_gaus/gray_images_iter:{iteration}")
