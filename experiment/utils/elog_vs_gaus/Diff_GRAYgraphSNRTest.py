...
def plot_values(iteration):
    ...
    ax1.plot(x, x1, 'b-o', label='Sharpness Differential-ELoG')  # Blue line with circle markers
    ax1.plot(x, x1_, 'r-s', label='Sharpness Laplacian')  # Red line with square markers
    ...
    ax2.plot(x, x2, 'g-^', label='SNR ELoG')  # Green line with triangle markers
    ax2.plot(x, x2_, 'm-+', label='SNR Gaussian')  # Magenta line with plus markers
    ...
    plt.tight_layout()
    #fig = plt.gcf()
    fig.savefig(f"elog_vs_gaus/diff_gray_plot_iter:{iteration}")
    ...
    plt.tight_layout()
    #fig = plt.gcf()
    fig.savefig(f"elog_vs_gaus/diff_gray_images_iter:{iteration}")
