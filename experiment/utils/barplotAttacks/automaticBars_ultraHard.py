...
def main():
    ...
    fig, ax = plt.subplots(figsize=(20+len(labels), 5))  # Increase horizontal size
    ...
    ax.legend(handles, legend_labels, loc='best', ncol=4, fontsize='xx-large')

    ax.axhline(y=offset, color='r', linestyle='--', linewidth=1)

    yticks = ax.get_yticks()
    new_ytick_labels = [""] + [int(tick - offset) for tick in yticks[1:]]
    ax.set_yticklabels(new_ytick_labels, fontsize=16, fontweight='bold')
    ax.grid(True, axis='y')
