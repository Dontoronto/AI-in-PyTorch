...
def tensor_saving_process(queue, convert_to_png, file_path='tensors.pkl'):
    if convert_to_png is True:
        tensor_buffer = TensorBuffer(capacity=5, file_path='mnt/data/frames_parallel',
                                     ...
                                     file_path_zero_matrices='mnt/data/zero_matrices')
    else:
...
def select_specific_tensor(tensors, list_index, batch_index, channel_index):
    """
    Returns a specific (3,3) tensor based on provided indices.

    Parameters:
    - tensors: List of tensors with shapes like (1,6,3,3) or (16,6,3,3).
    ...
    if list_index < 0 or list_index >= len(tensors):
        raise IndexError("list_index is out of bounds.")
    if batch_index < 0 or channel_index < 0:
        raise IndexError("batch_index or channel_index is out of bounds.")

    selected_tensor = tensors[list_index][batch_index, channel_index]

    # Further validation could be added to ensure the tensor has the expected shape
...
if __name__ == "__main__":
    ...
    second_directory_path = 'mnt/data/zero_matrices'

    gif_path = 'mnt/data/matrix_changes_colored.gif'  # Change this to your desired output GIF path
    create_two_matrix_gif(directory_path, second_directory_path, gif_path)
    ...
    #
    # print(select_specific_tensor(tensor_list,2,10,0))
    # print(len(tensor_list[2][10][0]))
