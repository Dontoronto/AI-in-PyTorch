...
def computationally_expensive_operation_parallel(tensor_queue):
    ...
    for i in range(10):  # Number of iterations to simulate heavy computation
        ...
        print(tensor.shape)
        # tensor_v2 = np.array([[0, 1, 2], [3, 4, 0], [6, 0, 9]])
        tensor_v2 = torch.tensor([[0, 1, 2], [3, 4, 0], [6, 0, 9]])
        print(tensor_v2.shape)
        tensor_queue.put([tensor, tensor_v2])
    end_time = time.time()
    tensor_queue.put(None)  # Signal to terminate the saving process
    return end_time - start_time
...
def create_two_matrix_gif(first_path, second_path, gif_path):
    ...
    with imageio.get_writer(gif_path, mode='I', duration=0.5) as writer:
        for filename_1, filename_2 in zip(filenames_1, filenames_2):
            ...
            #
            # # Convert PIL image to an array and append to the GIF
            # combined_image_array = imageio.core.util.Array(combined_image)
            # writer.append_data(combined_image_array)

            # Combining images side by side using numpy
...
if __name__ == "__main__":
    ...
    tensor_queue = Queue()
    process = Process(target=tensor_saving_process, args=(tensor_queue, False,))
    process.start()
