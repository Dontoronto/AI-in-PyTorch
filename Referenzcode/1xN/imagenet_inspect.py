...
def get_model(args):
    ...
    if args.arch == 'mobilenet_v2':
        fc_weight = ckpt['state_dict']['classifier.1.weight']
        ckpt['state_dict']['classifier.1.weight'] = fc_weight.view(
...
def save_all_weights_to_single_csv(model, filename):
    ...
    for name, param in model.named_parameters():
        ...
        # flattened_weights = param.data.view(-1).numpy()
        # # Create a DataFrame from the flattened weights
        # df = pd.DataFrame(flattened_weights, columns=[name])
        # # Concatenate to the main DataFrame
        # all_weights = pd.concat([all_weights, df], axis=1)
        counter += 1

    #     if counter == 20:
    #         # Handle layers that might have different numbers of parameters (fill missing values)
    #         all_weights.fillna(value=0, inplace=True)
    #
    #         # Save to CSV
    #         all_weights.to_csv(filename, index=False)
    #         return
    #
    #
    # # Handle layers that might have different numbers of parameters (fill missing values)
...
def main(args, var):
    ...
    if args.pruned_model == None:
        ...
        save_all_weights_to_single_csv(model, '/Users/dominik/Documents/jupyter/'
                                              'Neuronale Netze programmieren Buch/'
                                              'AI in PyTorch/Referenzcode/1xN/analyze/model_weights.csv')
...
class Args:
    def __init__(self, gpus, arch, job_dir, data_path, pretrained_model, pr_target, N, conv_type,
                 train_batch_size, eval_batch_size, rearrange, full):
        self.gpus = gpus
        self.arch = arch
        ...
        self.pruned_model = None  # Optional, defaults to None
        self.full = False

if __name__ == '__main__':
    args = Args(
        gpus=None,
        arch='mobilenet_v2',  # or 'mobilenet_v2', 'mobilenet_v3_large', 'mobilenet_v3_small'
        job_dir='./experiment/',
        # pretrained_model='/Users/dominik/Documents/jupyter/'
        #                  'Neuronale Netze programmieren Buch/'
        #                  'AI in PyTorch/Referenzcode/1xN/'
        #                  'pretrained/mobilenet_v2.pth.tar',  # Substitute your actual data path here
        pretrained_model='/Users/dominik/Documents/jupyter/'
                         'Neuronale Netze programmieren Buch/'
                         'AI in PyTorch/Referenzcode/1xN/'
                         'pretrained/pruned_rearranged/n8/'
                         'w_o_Rearrange/model_best.pt',
        data_path='[PRETRAIN_MODEL_PATH]',
        pr_target=0,
        N=4,  # or 2, 8, 16, 32
        conv_type='BlockL1Conv',
        train_batch_size=256,
        eval_batch_size=256,
        rearrange='',  # Add the actual rearrange value if required
        full=False
    )

    main(args, var=torch.rand(1,3,244,244))
